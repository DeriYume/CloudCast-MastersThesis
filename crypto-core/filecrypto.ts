...
function parseHeader(buf: Buffer): Parsed {
  if (buf.length < HEADER_LEN || !buf.subarray(0, MAGIC.length).equals(MAGIC)) {
    throw new Error("Unrecognized or corrupt CCE3 file.");
  }
...
export function encryptFile(plaintext: Buffer, dek: Buffer, chunkSize: number = DEFAULT_CHUNK): Buffer {
  return encryptFileWithPrefix(plaintext, dek, randomBytes(NONCE_PREFIX_LEN), chunkSize);
}

export function encryptFileWithPrefix(
  plaintext: Buffer, dek: Buffer, noncePrefix: Buffer, chunkSize: number = DEFAULT_CHUNK,
): Buffer {
  if (noncePrefix.length !== NONCE_PREFIX_LEN) throw new Error(`nonce prefix must be ${NONCE_PREFIX_LEN} bytes`);
...
export function decryptRange(cipher: Buffer, dek: Buffer, start: number, end: number): Buffer {
  const p = parseHeader(cipher);
  if (p.plaintextLen === 0) return Buffer.alloc(0);
  ...
  const parts: Buffer[] = [];
  for (let i = first; i <= last; i++) parts.push(decryptChunk(cipher, p, dek, i));
  const buf = Buffer.concat(parts);
  const s = start - first * p.chunkSize;
  return buf.subarray(s, s + (end - start) + 1);
}

export function plaintextLength(cipher: Buffer): number {
